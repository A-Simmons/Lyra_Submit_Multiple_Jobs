...
[ ! -f $rscript ] && { echo "Matlab script file $rscript could not be found"; exit 1; }
...
for (( i = 0;  i < $numjobs; i++ )); do
    ...
    echo executing: matlab -nodisplay -nosplash -r "${rscript//.m}($rargs)"
    matlab -nodisplay -nosplash -r "${rscript//.m}($rargs)"
done
